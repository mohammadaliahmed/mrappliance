...
import com.appsinventiv.mrappliance.Models.CouponModel;
import com.appsinventiv.mrappliance.Models.LogsModel;
import com.appsinventiv.mrappliance.Models.OrderModel;
import com.appsinventiv.mrappliance.Models.ServiceCountModel;
...
import com.appsinventiv.mrappliance.Utils.GetAddress;
            ...
            if (SharedPrefs.getUser().getGoogleAddress() != null && SharedPrefs.getUser().getGoogleAddress().equalsIgnoreCase("")) {
                ...
                mDatabase.child("Orders").child("" + orderId).setValue(model).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        CommonUtils.sendMessage(number, "MR APPLIANCE \nNew " + model.getServiceName() + " order \nOrder Id: " + orderId
                                + "\n\nClick to view: \n" + Constants.MR_APPLIANCE_URL + "admin/" + orderId);
                        ;
                        CommonUtils.sendMessage(SharedPrefs.getUser().getMobile(), "MR APPLIANCE\n\nOrder was successfully placed\n" +
...
//                                .child(ListOfSubServices.parentService)
                                ...
                                .child(ChooseServiceOptions.monthSelected)
                                .child(ChooseServiceOptions.daySelected)
                        ...
                        mDatabase.child("OrderLogs").child(SharedPrefs.getUser().getUsername()).child("" + orderId).child(key).setValue(new LogsModel(
                                key, "Order Placed", System.currentTimeMillis()
                        ));
    ...
    private long calculateTotal() {
        ...
        for (ServiceCountModel model : ListOfSubServices.orderList) {
            totalMinutes = totalMinutes + (model.getQuantity() * (model.getService().getTimeMin() + (model.getService().getTimeHour() * 60)));
        }
        ...
        if (couponOk) {
            ...
            orderDetails.setText("Total time will last for:" +
                    ...
                    " hours and \ncost shall be: AED" + finalTotalCost + " - AED" + (finalTotalCost
            )
            ...
            orderDetails.setText("Total time will last for:" +
                    " " + finalTotalTime + " - " + (finalTotalTime + 1) + "" +
                    " hours and \ncost shall be: AED" + finalTotalCost);
        }
